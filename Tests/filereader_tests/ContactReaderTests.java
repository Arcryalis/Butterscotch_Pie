...
public class ContactReaderTests {
    ...
    public void Test2a () {
		ContactsReader reader = new ContactsReader();
		try {
			assertEquals(reader.getRequests(SECOND_ACCOUNT.getUsername(), true).contains("u3"), false);
		} catch (Exception e) {
    ...
    public void Test2b () {
		ContactsReader reader = new ContactsReader();
		try {
			Boolean truth = reader.getRequests(SECOND_ACCOUNT.getUsername(), false).contains("u3");
			assertEquals(truth, true);
		} catch (Exception e) {
    ...
    public void Test3a () {
		...
		try {
			reader.updateContacts(SECOND_ACCOUNT.getUsername(), FOURTH_ACCOUNT.getUsername(), true);
		} catch (Exception e) {
			System.out.println(e + " FAIL Test3a add");
		}	
		
		try {
			reader.updateContacts(SECOND_ACCOUNT.getUsername(), FOURTH_ACCOUNT.getUsername(), false);
		} catch (Exception e) {
			System.out.println(e + " FAIL Test3a update");
		}	
		
		try {
			reader.delete(SECOND_ACCOUNT.getUsername(), FOURTH_ACCOUNT.getUsername());
		} catch (Exception e) {
    ...
    public void Test4a () {
		...
		try {
			...
			assertEquals(truth.getUsername(), SECOND_ACCOUNT.getUsername());
		} catch (Exception e) {
